...
def register_face():
    ...
    # Create directory for person (be robust if a file exists with same name)
    person_dir = os.path.join(FACES_DB_PATH, name)
    try:
        if os.path.exists(person_dir):
            if not os.path.isdir(person_dir):
                # A non-directory file exists at this path — remove it and create dir
                print(f"Warning: non-directory exists at {person_dir}, replacing with directory")
                os.remove(person_dir)
                os.makedirs(person_dir, exist_ok=True)
        else:
            os.makedirs(person_dir, exist_ok=True)
    except Exception as e:
        print(f"Error creating person directory {person_dir}: {e}")
        return jsonify({"error": "Failed to create person directory", "details": str(e)}), 500

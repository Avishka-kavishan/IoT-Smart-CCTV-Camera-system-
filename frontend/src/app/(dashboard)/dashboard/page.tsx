...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchBackendStats = async () => {
      try {
        const res = await fetch(`${ENV.BACKEND_URL}/stats`);
        if (res.ok) {
          ...
          setStats((prev) =>
            prev.map((stat) => {
              if (stat.name === "Known Persons") {
                return {
                  ...stat,
                  value: data.known_persons.toString(),
                  change: "Registered",
                };
              }
              if (stat.name === "Active Cameras") {
                return {
                  ...stat,
                  value: data.active_cameras.toString(),
                  change: "Online",
                };
              }
              return stat;
    ...
    const fetchFirestoreStats = async () => {
      ...
      const unsubscribeStats = onSnapshot(alertsTodayQuery, (snapshot) => {
        ...
        const unknownsToday = snapshot.docs.filter(
          (doc) => doc.data().status === "unknown"
        ).length;
      ...
      snapshot.docs.forEach((doc) => {
        ...
        csvRows.push([
          ...
          data.details?.confidence
            ? `${data.details.confidence.toFixed(2)}%`
            : "N/A",
          `"${data.cameraName || "N/A"}"`,
        ]);
      });
            ...
            {recentAlerts.length === 0 ? (
              ...
              recentAlerts.map((alert) => (
                ...
                >
                  ...
                  <Badge
                    variant={
                      alert.details?.name && alert.details.name !== "Unknown"
                        ? "success"
                        : "danger"
                    }
                  >
